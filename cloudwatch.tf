...
resource "aws_cloudwatch_metric_alarm" "kafka-cpu-alarm" {
  ...
  alarm_name = format("%s-kafka-cpu-%s-%02d", var.environment, element(data.aws_subnet.subnet.*.availability_zone, count.index), floor((count.index/length(data.aws_subnet.subnet))+1))
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods = "5"
  namespace = "AWS/EC2"
  metric_name = "CPUUtilization"
...
resource "aws_cloudwatch_metric_alarm" "kafka-status-alarm" {
  count = length(aws_instance.kafka-server)
  alarm_name = format("%s-kafka-status-%s-%02d", var.environment, element(data.aws_subnet.subnet.*.availability_zone, count.index), floor((count.index/length(data.aws_subnet.subnet))+1))
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods = "1"
  namespace = "AWS/EC2"
  metric_name = "StatusCheckFailed"
...
resource "aws_cloudwatch_metric_alarm" "kafka-proc-alarm" {
  count = length(aws_instance.kafka-server)
  alarm_name = format("%s-kafka-proc-%s-%02d", var.environment, element(data.aws_subnet.subnet.*.availability_zone, count.index), floor((count.index/length(data.aws_subnet.subnet))+1))
  comparison_operator = "LessThanThreshold"
  evaluation_periods = "1"
  namespace = "CMXAM/Kafka"
